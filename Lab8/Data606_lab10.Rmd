...
#### Ans) A scatterplot seems best suited to display the relationship between runs and another variables.
...
#### Ans) There appears to be a positive relationship between runs and at bats.
####      A R^2 of 0.610627 indicates a positive correlation but one that is not very strong.
####      There appears to be a great deal of deviation amongst the teams and the points are not centered around a             specific value.
...
#### Ans) The value of sum of squared residuals will change as we run multiple iterations of this function by              selecting any 2 pooints every time. To find the minimum squared line, we will use a built-in argument of this        function, which is leastSquares = TRUE. It will give the line which is the best linear model in this respect.
```{r}
...
#### Based on the output from above, in the coefficients table, the first column gives the intercept and the slope         respectively from the first row and 2nd row of the table. Hence the equation for the least squared regression         line will be:
...
```
#### In the actual data set, there is no entry for the ab_stats value 5578. However, there is a very close value          which is 5579. The runs for this value is 713. Hence the predicted value > actual value
#### So, residual is: observed value - predicted value = 713 - 727.69 = -24.69.
...
#### Ans) Based on the scatterplot, most of the residuals are centered around 0, indicating linearity.
...
#### Ans) Based on the histogram and normal probability plot, the residuals appear to be normal.
...
#### Ans) The constant variability condition does appear to be met.
...
#### Yes, it seems to be a linear relationship.
