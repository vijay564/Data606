...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    Ans) The streak length of 1 would mean that Kobe made one basket i.e, 1 Hit and didnot make any baskets after that. The Streak length of 0 means that Kobe missed a basket.
...
Ans) The distribution is right skewed as the number of counts decrease when the streak length increases. Kobe's typical streak was 0, which would be an instance where he missed a basket. His longest streak was 4 baskets in a row.
...
```{r}
# Identify number of heads and tails for unfair coin
...
Ans)
We need to adjust the Hit percentage to 45 and do simulation for 133 sample shots
...
Ans) The ditribution is right skewed. The typical streak length is still at 0, but the longest streak has increased to five.
...
Ans) Even when we run the simulation for second time i would expect the distribution to be similar. Since we are drawing a random sample from a distribution, it wouldn’t be exactly the same, but because the events are independent in the simulation and the probabilities do not change, I expect the distributions to be similar or somewhat larger with the sample of 133.
...
Ans) Kobe'sdistribution of streak lengths is similar to the streak lengths of simulated shooter. By this there is no evidence that hot hand model fitls Kobe's shooting pattern. Based on this we can assume that Kobe’s individual shots are independent of the previous shot.