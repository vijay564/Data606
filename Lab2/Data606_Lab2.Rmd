...
Ans : 
There are 20000 rows in this dataset which has 9 variables and they can be categorized as the following: 
...
# Relative frequency for exerany
...
# Proportion of sample reports in excellent health
...
Ans : The mosaic plot shows there are more males than females reported having smoked 100 cigarettes in their lifetime.
...
Ans : Based on this We can find that bmi is inversely correlated with reported health condition.

Lets check if bmi is also negatively correlated with exerany.
```{r}

boxplot(bmi ~ cdc$exerany)
```

From this we can observer bmi is negatively correlated.
...
#### The 2nd graph is much more descriptive. The 1st histogram grouped data into large range which is difficult to read the results
...
The desired weight of those with a higher weight is higher than those with a lower weight
...
Ans)  wdiff is a Numerical discrete variable. If wdiff is 0, then the person's actual and desired weight are equal is at their desired weight. If wdiff is negative then the person wants to lose weight. If wdiff is positive then the person wants to gain weight.
...
Based on this we can figure out that people want to lose weight
...
```{r}
gender_wdiff <- data.frame(wdiff, cdc$gender)
boxplot(gender_wdiff$wdiff ~ gender_wdiff$cdc.gender)

summary(subset(gender_wdiff, cdc.gender == "m"))
summary(subset(gender_wdiff, cdc.gender == "f"))

```

From this we can figure out that men are closer to their ideal weight, while women prefer to lose more weight.
...
sd <- sd(cdc$weight)
(round(sd,2))
instdev <- subset(cdc, weight < (mn + sd) & weight > (mn - sd))
